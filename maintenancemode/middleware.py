...
class MaintenanceModeMiddleware(object):
    def process_request(self, request):
        if not request.path_info.startswith(settings.MEDIA_URL) and \
            not request.path_info.startswith(settings.ADMIN_MEDIA_PREFIX) and \
            not request.path_info.startswith('/admin'): #should be changed to be independent of where the admin is set up
